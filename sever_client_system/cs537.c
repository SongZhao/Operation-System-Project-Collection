...
void getargs2(struct server_t* server_reg,int argc, char *argv[])
{
    if (argc != 4) {
    ...
    printf("Port = %d\nThreads = %d\nBuffer = %d\n",server_reg->port,server_reg->thread_num,server_reg->buffer_sz);
}

void put(int value,struct server_t* server_reg,char* buff,int file_sz,int filename_sz) {
 int i=0;
 ...
 int get(struct server_t* server_reg,char** buff_ptr) {
 int tmp; 
 int i=0;
