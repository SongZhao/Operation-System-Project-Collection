...
void* Mem_Alloc(int size)
{
  /* Your code should go in here */
    ...
    block_header* temp_head = list_head;
    while(itr != NULL){    
        if(itr->size_status < size){
            itr=itr->next;
          ...
          //test
            ...
            if(itr->size_status <= size+sizeof(block_header)){
            //allocate
                itr->size_status = itr->size_status-1;
                int* location = (int*) itr;
                return location;
            }
            else {
                //allocate, partition
                ...
                char* location2 = (char*)itr;
                block_header* new_head = (block_header*) ((location2)+(itr->size_status)+sizeof(block_header));
                block_header* new_header = (block_header*) new_head;
                block_header* ptr = (block_header*) ((char*)new_header-(char*)sizeof(block_header));
                ptr->size_status = itrSave - size; 
                //test
...
int Mem_Free(void *ptr)
{
  /* Your code should go in here */
    ...
    while(itr != NULL){
        ...
        if(itr == ptrLoc/*+(char*)sizeof(block_header)*/){
            //check if it is not allocated yet
            if(itr->size_status%4 == 0) {return -1;}
            ...
            while(newItr != NULL){
                if(newItr->size_status%4 == 0 && prevItr->size_status%4 == 0){
                    prevItr->size_status =  ptrLoc->size_status + itr->size_status + sizeof(block_header) + sizeof(block_header);
